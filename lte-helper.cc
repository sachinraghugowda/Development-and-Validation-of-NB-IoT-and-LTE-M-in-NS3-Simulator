...
/** NetDeviceContainer
LteHelper::InstallNbEnbDevice (NodeContainer c)                                         //monisha no need
{
  NS_LOG_FUNCTION (this);
  Initialize ();  // will run DoInitialize () if necessary
  NetDeviceContainer devices;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleNbEnbDevice (node);
      devices.Add (device);
    }
  return devices;
}     **/                                                                                    //end
...
NetDeviceContainer                                                              //monisha
LteHelper::InstallNbUeDevice (NodeContainer c)
{
  NS_LOG_FUNCTION (this);
  NetDeviceContainer devices;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleNbUeDevice (node);
      devices.Add (device);
    }
  return devices;
}                                                                               //end
...
/**
Ptr<NetDevice>
LteHelper::InstallSingleNbEnbDevice (Ptr<Node> n)                                               //monisha
{
...
Ptr<NetDevice>
LteHelper::InstallSingleNbUeDevice (Ptr<Node> n)                                        //monisha
{
  NS_LOG_FUNCTION (this);
  Ptr<LteSpectrumPhy> dlPhy = CreateObject<LteSpectrumPhy> ();
  Ptr<LteSpectrumPhy> ulPhy = CreateObject<LteSpectrumPhy> ();
...
void                                                                                                                         //sachin start
LteHelper::NbAttach (NetDeviceContainer nbueDevices)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = nbueDevices.Begin (); i != nbueDevices.End (); ++i)
    {
      NbAttach (*i);
    }
}                                                                                                                            //sachin end
...
void                                                                                                               //sachin start
LteHelper::NbAttach (Ptr<NetDevice> nbueDevice)
{
  NS_LOG_FUNCTION (this);
  ...
  Ptr<LteUeNetDevice> nbueLteDevice = nbueDevice->GetObject<LteUeNetDevice> ();
  if (nbueLteDevice == 0)
    {
      NS_FATAL_ERROR ("The passed NetDevice must be an LteUeNetDevice");
    }
  ...
  // initiate cell selection
  Ptr<EpcUeNas> ueNas = nbueLteDevice->GetNas ();
  NS_ASSERT (ueNas != 0);
  uint16_t dlEarfcn = nbueLteDevice->GetDlEarfcn ();
  ueNas->StartCellSelection (dlEarfcn);
  ...
  // activate default EPS bearer
  m_epcHelper->ActivateEpsBearer (nbueDevice, nbueLteDevice->GetImsi (),
                                  EpcTft::Default (),
                                  EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT));
}
                                                                                                                //sachin end
...
void                                                                                                        //sachin start
LteHelper::NbAttach (NetDeviceContainer nbueDevices, Ptr<NetDevice> enbDevice)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = nbueDevices.Begin (); i != nbueDevices.End (); ++i)
    {
      NbAttach (*i, enbDevice);
    }
}                                                                                                            //sachin end
...
void                                                                                                                              //sachin start
LteHelper::NbAttach (Ptr<NetDevice> nbueDevice, Ptr<NetDevice> enbDevice)
{
  NS_LOG_FUNCTION (this);
  //enbRrc->SetCellId (enbDevice->GetObject<LteEnbNetDevice> ()->GetCellId ());
  ...
  if (m_epcHelper != 0)
    {
      // activate default EPS bearer
      m_epcHelper->ActivateEpsBearer (nbueDevice, nbueLteDevice->GetImsi (), EpcTft::Default (), EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT));
    }

  // tricks needed for the simplified LTE-only simulations 
  if (m_epcHelper == 0)
    {
      nbueDevice->GetObject<LteUeNetDevice> ()->SetTargetEnb (enbDevice->GetObject<LteEnbNetDevice> ());
    }
}
                                                                                                                                  //sachin end
...
void                                                                                                                        //sachin start
LteHelper::NbAttachToClosestEnb (NetDeviceContainer nbueDevices, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = nbueDevices.Begin (); i != nbueDevices.End (); ++i)
    {
      NbAttachToClosestEnb (*i, enbDevices);
    }
}                                                                                                                          //sachin end
...
void                                                                                                                      //sachin start
LteHelper::NbAttachToClosestEnb (Ptr<NetDevice> nbueDevice, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (enbDevices.GetN () > 0, "empty enb device container");
  Vector uepos = nbueDevice->GetNode ()->GetObject<MobilityModel> ()->GetPosition ();
  double minDistance = std::numeric_limits<double>::infinity ();
  Ptr<NetDevice> closestEnbDevice;
  for (NetDeviceContainer::Iterator i = enbDevices.Begin (); i != enbDevices.End (); ++i)
    {
  ...
  NS_ASSERT (closestEnbDevice != 0);
  NbAttach (nbueDevice, closestEnbDevice);
}                                                                                                                        //sachin end
...
/**
uint8_t                                                                                                                    //sachin start
LteHelper::ActivateDedicatedEpsBearer (NetDeviceContainer nbueDevices, EpsBearer bearer, Ptr<EpcTft> tft)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = nbueDevices.Begin (); i != nbueDevices.End (); ++i)
    {
...
/**
uint8_t                                                                                                            //sachin start
LteHelper::ActivateDedicatedEpsBearer (Ptr<NetDevice> nbueDevice, EpsBearer bearer, Ptr<EpcTft> tft)
{
  NS_LOG_FUNCTION (this);

  NS_ASSERT_MSG (m_epcHelper != 0, "dedicated EPS bearers cannot be set up when the EPC is not used");

  uint64_t imsi = nbueDevice->GetObject<LteUeNetDevice> ()->GetImsi ();
  uint8_t bearerId = m_epcHelper->ActivateEpsBearer (ueDevice, imsi, tft, bearer);
  return bearerId;
}                                                                                                                 //sachin end
**/
...
private:
 ...
 Ptr<NetDevice> m_nbueDevice;                                                                    //sachin
  /**
