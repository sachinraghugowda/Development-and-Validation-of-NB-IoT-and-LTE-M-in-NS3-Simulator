...
/**
void                                                                                                       //sachin start
PointToPointEpcHelper::AddNbue (Ptr<NetDevice> nbueDevice, uint64_t imsi)
{
  NS_LOG_FUNCTION (this << imsi << nbueDevice );
  
  m_mme->AddNbue (imsi);
  m_sgwPgwApp->AddNbue (imsi);
...
PointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> nbueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this << nbueDevice << imsi);

  // we now retrieve the IPv4 address of the UE and notify it to the SGW;
  // we couldn't do it before since address assignment is triggered by
  // the user simulation program, rather than done by the EPC   
  Ptr<Node> nbueNode = nbueDevice->GetNode (); 
  Ptr<Ipv4> nbueIpv4 = nbueNode->GetObject<Ipv4> ();
  NS_ASSERT_MSG (nbueIpv4 != 0, "NBUEs need to have IPv4 installed before EPS bearers can be activated");
  int32_t interface =  nbueIpv4->GetInterfaceForDevice (nbueDevice);
  NS_ASSERT (interface >= 0);
  NS_ASSERT (nbueIpv4->GetNAddresses (interface) == 1);
  Ipv4Address ueAddr = nbueIpv4->GetAddress (interface, 0).GetLocal ();
  NS_LOG_LOGIC (" UE IP address: " << ueAddr);  m_sgwPgwApp->SetUeAddress (imsi, ueAddr);
  ...
  if (nbueLteDevice)
    {
      nbueLteDevice->GetNas ()->ActivateEpsBearer (bearer, tft);
    }
  return bearerId;
}
                                                                                                                                      //sachin end
...
Ipv4InterfaceContainer                                                                        //sachin start 
PointToPointEpcHelper::AssignNbueIpv4Address (NetDeviceContainer nbueDevices)
{
  return m_ueAddressHelper.Assign (nbueDevices);
}                                                                                             //sachin end
